...
void resolve_flag_conflicts(int *flags) {
    ...
    if (IS_SET(*flags, v_flag) ||
        IS_SET(*flags, c_flag) ||
        IS_SET(*flags, l_flag)) {
        CLEAR_BIT(*flags, o_flag);
    }
}
...
void handle_grep_output_no_o(char **files, int flagset, struct grep_config *c) {
    if (!IS_SET(flagset, o_flag)) {
        if (c->files_n > 1 && !IS_SET(flagset, h_flag) &&
            !IS_SET(flagset, c_flag) && !IS_SET(flagset, l_flag)) {
            fprintf(stdout, "%s:", files[c->cur_file]);
        }

        if (IS_SET(flagset, n_flag) && !IS_SET(flagset, c_flag) &&
            !IS_SET(flagset, l_flag)) {
            fprintf(stdout, "%d:", c->cur_line);
            }

        if (!IS_SET(flagset, c_flag) && !IS_SET(flagset, l_flag)) {
...
void handle_grep_post_flags(struct grep_config *conf, char **files, int flagset) {
    if (IS_SET(flagset, c_flag) && conf->non_empty) {
        if (conf->files_n > 1 && !IS_SET(flagset, h_flag)) {
            fprintf(stdout, "%s:", files[conf->cur_file]);
        }

        if (IS_SET(flagset, l_flag) && conf->total_matches > 1) {
    ...
    if (IS_SET(flagset, l_flag) && conf->total_matches > 0) {
        fprintf(stdout, "%s\n", files[conf->cur_file]);
    }
    conf->cur_file++;
}
...
int match_regex_ll(reg_node_t *root, char *line, int flagset, regmatch_t *offset) {
    ...
    return IS_SET(flagset, v_flag) ? !match : match;
}
...
void print_debug(int flagset, char **files, char **pfiles, char **inline_patterns) {
    ...
    if (IS_SET(flagset, v_flag)) {
        printf("-v - Inverse \n");
    }
    if (IS_SET(flagset, c_flag)) {
        printf("-c - Match count flag \n");
    }
    if (IS_SET(flagset, l_flag)) {
        printf("-l - Matching file names  \n");
    }
    if (IS_SET(flagset, n_flag)) {
        printf("-n - Number lines  \n");
    }
    if (IS_SET(flagset, h_flag)) {
...
void parse_grep_flags(const char **argv, int *flag_set, int argc) {
    for (int i = 0; i < argc; i++) {
        ...
        if (strspn(arg, "-") == 1) {
            if (strchr(arg, 'e')) {
                SET_BIT(*flag_set, e_flag);
            }
            if (strchr(arg, 'i')) {
                SET_BIT(*flag_set, i_flag);
            }
            if (strchr(arg, 'v')) {
                SET_BIT(*flag_set, v_flag);
            }
            if (strchr(arg, 'c')) {
                SET_BIT(*flag_set, c_flag);
            }
            if (strchr(arg, 'l')) {
                SET_BIT(*flag_set, l_flag);
            }
            if (strchr(arg, 'n')) {
                SET_BIT(*flag_set, n_flag);
            }
            if (strchr(arg, 'h')) {
                SET_BIT(*flag_set, h_flag);
            }
            if (strchr(arg, 's')) {
                SET_BIT(*flag_set, s_flag);
            }
            if (strchr(arg, 'f')) {
                SET_BIT(*flag_set, f_flag);
            }
            if (strchr(arg, 'o')) {
                SET_BIT(*flag_set, o_flag);
            }
        }
    }
